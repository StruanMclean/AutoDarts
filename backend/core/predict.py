...
from dartboard import DartboardScorer
...
class Predict:
    def __init__(self):
        ...
        cfg.MODEL.WEIGHTS = "/Users/struanmclean/Documents/DartsProject/backend/model/model_final.pth"
    ...
    def compute_homography(self, calib_points, dartboard_center: Vector):
        if len(calib_points) < 4:
            print(f"Need exactly 4 calibration points for homography, got {len(calib_points)}")
            return None

        outer_radius = self.scorer.double_outer_radius 
        
        print(f"Using outer radius: {outer_radius} for homography computation")
        ...
        try:
            ...
            print(f"Missing calibration point: {e}")
            return None
    ...
    def apply_nms_to_darts(self, darts: List[Vector], scores: List[float], 
                          nms_threshold: float = 0.3) -> List[Vector]:
        if len(darts) <= 1:
            return darts
    ...
    @staticmethod
    def __get_point_on_dartboard(point: Vector, homography: np.ndarray) -> Vector:
        image_point = np.array([[[point.x, point.y]]], dtype=np.float32)
        dartboard_point = cv2.perspectiveTransform(image_point, homography)

        return Vector(dartboard_point[0][0][0], dartboard_point[0][0][1])
    ...
    def run(self, cam: Camera):
        ...
        scores = []
        if new_darts:
            print(f"Found {len(new_darts)} new darts!")
